	...
	def getvectors(self, vectors):
		...
		for x in range(0,360/scale):
			for y in range(0,640/scale):
				...
				totalx += vectors[scale*x][scale*y][0]
				totaly += vectors[scale*x][scale*y][1]

		totalx /= 230400/scale^2
		totaly /= 230400/scale^2

		for x in range(0,360/scale):
			for y in range(0,640/scale):

				vectors[scale*x][scale*y][0] -= totalx #turns image vectors into vectors relative to the drone's overall motion
				vectors[scale*x][scale*y][1] -= totaly

				dotprod = vectors[x][y][0]*(x-180/scale) + vectors[x][y][1]*(x-320/scale)
				totalrad += dotprod#/np.sqrt((x-180/scale)^2+(y-320/scale)^2)

		print totalx
		print totaly
		print totalrad/scale^2
