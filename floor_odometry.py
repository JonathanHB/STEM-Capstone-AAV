...
class Hoop_finder:
	...
	def processimage2(self, imgdrone, data):
		...
		cv2.circle(self.mask, (int(20*kp*self.v[2]*np.cos(self.v[4][0])+self.ctrx), int(20*kp*self.v[3]*np.sin(self.v[4][0])+self.ctry)), 1, (0,0,127), 2)
		...
		twist = Twist()
		twist.linear.x = kp*self.v[2]*np.cos(self.v[4][0]); twist.linear.y = kp*self.v[3]*np.sin(self.v[4][0]); twist.linear.z = 0; twist.angular.x; twist.angular.y = 0; twist.angular.z = 0 #currently does both axial and lateral stabilization, doesn't move
		self.pub_twist.publish(twist) 
