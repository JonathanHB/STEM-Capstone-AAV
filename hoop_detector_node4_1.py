...
class Hoop_finder:
	...
	def __init__(self):
		...
		self.Kvals = ((-1,0,0),(.1,-.01,0),(.12,0,0)) #the azimuthal, axial, and lateral PID constants #y:d = -.005
		...
		self.rollavglength = 30
		self.rollingavgdata = [0.0]*self.rollavglength		
		self.polyfitarg = [0.0]*self.rollavglength
	...
	def hoopnav(self, pose):
		...
		if(pose[0] != -1 or pose[1] != -1 or pose[2] != -1): #if a hoop is actually detected
			...
			if self.vy != 0 and avgderiv != 0:
				self.direction = -np.sign(avgderiv)*np.sign(self.vy)
	...
	def processimage1(self, imgdrone1):
		...
		if(len(contour_area)>1):
			...
			if len(stacked)>100:# and len(contour_area[0][1])>40 and len(contour_area[1][1])>40:
				...
				hoop_angle2 = (ellipse[1][1]-ellipse[1][0])/ellipse[1][1] #computes a useful proxy for hoop angle which approaches 0 better than the inverse cosine
