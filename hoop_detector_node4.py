...
class Hoop_finder:
	...
	def __init__(self):
		...
		self.sub_image = rospy.Subscriber("/ardrone/odometry", Odometry, self.odometry, queue_size=1)
		...
		self.rollavglength = 5
		self.rollingavgdata = []*self.rollavglength		
		self.polyfitarg = []*self.rollavglength
	...
	def gen_polyfit_arg(self):

		for i in range(0,self.rollavglength-1):

			self.polyfitarg[i] = i

	def update_roll_avg(self, newderiv):

		for i in range(1,self.rollavglength-1):

			self.rollingavgdata[i] = self.rollingavgdata[i-1]

		self.rollingavgdata[0] = newderiv

	def get_roll_avg(self):

		m,b = np.polyfit(polyfitarg, self.rollingavgdata)
	...
	def hoopnav(self, pose):
		...
		if(pose[0] != -1 or pose[1] != -1 or pose[2] != -1): #if a hoop is actually detected
			...
			self.update_roll_avg(self.PDIvals[2][1])
			...
			#self.revcount+=1
			if self.vy != 0 and self.PDIvals[2][1] != 0:
				self.direction = -np.sign(self.PDIvals[2][1])*np.sign(self.vy)
			...
			y *= self.direction #doesn't flip flop constantly when direction is negative bcz y is recalculated each time
	...
	def odometry(self, imudata):
		...
		#print imudata
		#print imudata.twist.twist.linear.y
		self.vy = imudata.twist.twist.linear.y
